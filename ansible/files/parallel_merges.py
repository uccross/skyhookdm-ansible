...
def driver(
  ...
  if merge_type == "copyfrom" :
    ...
    cmd = "bin/run-client-merge" + \
          " --pool " + poolname + \
          " --start-oid " + start_oid + \
          " --end-oid " + end_oid + \
          " --merge-id " + merge_id + \
          " --project-cols " + PROJECT_COLS_LINEITEM + \
          " --data-schema \"" + DATA_SCHEMA_LINEITEM + "\"" + \
          " --debug false"
  else :
    print "unrecognized merge_type '" + merge_type + "'"
  print cmd
  if not cmd.startswith( "bin" ) :
    print "command is empty! aborting..."
    sys.exit( 0 )
  os.system( cmd )
...
for i in range( 0, num_merge_objs ) :
  job_args = ( poolname, str( start_oid ), str( end_oid ), str( i ), merge_type, test_name )
  print "======================================================"
  print job_args
  print "======================================================"
